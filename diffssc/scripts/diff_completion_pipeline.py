...
import models.minkunet as minknet
...
def main(diff, refine, denoising_steps, cond_weight):
    ...
    input_path = 'SemanticKitti/dataset/sequences/08/velodyne/'
    input_copy_path = 'DiffSSC/var/input/'
    diff_path = 'DiffSSC/var/diff/'
    refine_path = 'DiffSSC/var/refine/'


    os.makedirs(diff_path, exist_ok=True)
    os.makedirs(refine_path, exist_ok=True)

    file_list = natsorted(os.listdir(input_path))
    for i in tqdm.tqdm(range(0, len(file_list), 5)):
        pcd_path = file_list[i]
    # for pcd_path in tqdm.tqdm(natsorted(os.listdir(input_path))):
        pcd_file = os.path.join(input_path, pcd_path)
        points = read_bin(pcd_file)
        ...
        pcd_input.estimate_normals()
        o3d.io.write_point_cloud(input_copy_path + f'{pcd_path.split(".")[0]}.ply', pcd_input)

        pcd_diff = o3d.geometry.PointCloud()
        pcd_diff.points = o3d.utility.Vector3dVector(diff_scan)
        pcd_diff.estimate_normals()
        o3d.io.write_point_cloud(diff_path + f'{pcd_path.split(".")[0]}.ply', pcd_diff)

        pcd_refine = o3d.geometry.PointCloud()
        pcd_refine.points = o3d.utility.Vector3dVector(refine_scan)
        pcd_refine.estimate_normals()
        o3d.io.write_point_cloud(refine_path + f'{pcd_path.split(".")[0]}.ply', pcd_refine)
