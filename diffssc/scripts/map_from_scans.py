...
@click.command()
@click.option('--read_path', '-p', type=str, help='path to read the scan sequence')
@click.option('--write_path', '-p', type=str, help='path to write the geoundtruth')
@click.option('--voxel_size', '-v', type=float, default=0.1, help='voxel size')
@click.option('--cpu', '-c', is_flag=True, help='Use CPU')
def main(read_path, write_path, voxel_size, cpu):
    ...
    for seq in ['00','01','02','03','04','05','06','07','08','09','10']:
        ...
        poses = load_poses(os.path.join(read_path, seq, 'calib.txt'), os.path.join(read_path, seq, 'poses.txt'))
        for pose, pcd_path in tqdm.tqdm(list(zip(poses, natsorted(os.listdir(os.path.join(read_path, seq, 'velodyne')))))):
            pose = torch.from_numpy(pose).float().to(device)
            pcd_file = os.path.join(read_path, seq, 'velodyne', pcd_path)
        ...
        os.makedirs(os.path.join(write_path, seq), exist_ok=True)
        print(f'saving map for sequence {seq}')
        np.save(os.path.join(write_path, seq, 'clean_map.npy'), map_points.cpu().numpy())
