...
from datasets.utils.pcd_preprocess import point_set_to_coord_feats, aggregate_pcds, load_poses
from datasets.utils.pcd_transforms import *
# from datasets.utils.data_map import learning_map
from datasets.utils.collations import point_set_to_sparse
...
class TemporalKITTISet(Dataset):
    def __init__(self, data_dir, gt_dir, seqs, split, resolution, num_points, max_range, dataset_norm=False, std_axis_norm=False):
        super().__init__()
    ...
    def datapath_list(self):
        ...
        for seq in self.seqs:
            ...
            gt_seq_path = os.path.join(self.gt_dir, seq)
            ...
            p_full = np.load(f'{gt_seq_path}/clean_map.npy') if self.split != 'test' else np.array([[1,0,0],[0,1,0],[0,0,1]])
